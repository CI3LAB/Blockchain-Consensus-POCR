...
var test_request_num uint64 = 0

func (n *Node) executeAndReplyThread() {
	for {
		select {
		case <-n.executeNotify:
			log.Printf("[Reply] start execute for sequence (%d) and last sequence is (%d)", int(n.sequence.sequence), int(n.sequence.lastSequence))
			// execute batch
			...
			if n.sequence.ReadyToCheckPoint() {
				...
				n.BroadCast(content, httpserver.CheckPointEntry)
			}
			...
			test_request_num = test_request_num + uint64(len(requestBatchs))
			log.Printf("[Reply] set last sequence(%d) already execute request(%d)", lastSeq, test_request_num)
			// pending state
